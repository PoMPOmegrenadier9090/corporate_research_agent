...
def run_list(page_size: int, title_contains: str | None) -> dict[str, Any]:
    return task_db.action_list_records(page_size=page_size, title_contains=title_contains)
...
def main() -> None:
    ...
    try:
        if action == "list":
            result = run_list(page_size=getattr(args, "page_size", 20), title_contains=getattr(args, "title_contains", None))
        elif action == "add":
